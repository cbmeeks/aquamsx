...
//    int flags = SDL_SWSURFACE | SDL_FULLSCREEN; /* MK2 */
    int bytepp;
...
//    int flags = SDL_OPENGL | SDL_FULLSCREEN; /* MK2 */

	SDL_GL_SetAttribute(SDL_GL_SWAP_CONTROL, 1); // MK2

	SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1);

//	surface = SDL_SetVideoMode(width, height, fullscreen ? bitDepth : 0, flags);

	surface = SDL_SetVideoMode(width, height, 32, flags); /* MK2 */
	...
	if (bitDepth == 16) {
		glTexImage2D(GL_TEXTURE_2D, 0, GL_BGR, texW, texH, 0,
			            GL_BGR, GL_UNSIGNED_SHORT_5_6_5, displayData[0]);
	} 
    else {
...
int createSdlWindow()
{
    const char *title = "blueMSXlite X";
            ...
            if (updateAll || isLineDirty(y, linesPerBlock)) {
                ...
                if (bitDepth == 16) {
		            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, y, width, linesPerBlock,
		                            GL_BGR, GL_UNSIGNED_SHORT_5_6_5, displayData[curDisplayData] + y * displayPitch);
	            } 
                else {
		            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, y, width, linesPerBlock,
		                            GL_BGRA, GL_UNSIGNED_BYTE, displayData[curDisplayData] + y * displayPitch);
	            }
            }
        }
